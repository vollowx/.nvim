...
local lines = {
  {
    chunks = {
      ...
      {
        text = ' - Reactions appear immediately',
        hl = 'NonText',
      },
    },
  },
...
for _, line in ipairs(lines) do
  ...
  if line.width > win_config.width then win_config.width = line.width end
end
...
if win_config.row < 4 or win_config.col < 8 then return end
...
vim.api.nvim_buf_set_lines(
  ...
  false,
  vim.tbl_map(
    function(line) return string.rep(' ', line.offset) .. line.text end,
    lines
  )
)
