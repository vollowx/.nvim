...
function builders.signcol(data, filter, virtual)
  if not data.show_scl then return '' end
  if data.virtnum ~= 0 and not virtual then goto signcol_ret_default end
  do
    ...
    for _, sign in ipairs(data.extsigns) do
      ...
      if lnum > data.lnum then break end
      if
        lnum == data.lnum
...
function builders.lnum(data)
  ...
  if not data.show_nu then return '' end
...
function builders.foldcol(data)
  if not data.show_fdc then return '' end
...
local function gitsigns_filter(sign, data)
  ...
  if not name:find '^Git' then return false end
