...
local function setup_keymaps(_, bufnr)
  ...
  map('n', 'gEl',        vim.diagnostic.setloclist, { buffer = bufnr, desc = 'lsp: Set local list' })
  map('n', 'gEq',        vim.diagnostic.setqflist,  { buffer = bufnr, desc = 'lsp: Set QF list' })
  ...
  map('n', 'K',  if_supports('textDocument/hover', 'hover', 'K'),                     { buffer = bufnr, expr = true, desc = 'lsp: Hover' })
  -- stylua: ignore end
end
