...
vim.api.nvim_create_autocmd('FileChangedShellPost', {
  ...
  callback = function() path_branches = {} end,
})
...
vim.api.nvim_create_autocmd('BufWrite', {
  ...
  callback = function(info) buf_diffstats[info.buf] = nil end,
})
...
function M.diffstat(buf)
  ...
  local dir = vim.fs.dirname(path)
  if not dir or M.branch(path):find '^%s*$' then return {} end
  local diffstat = buf_diffstats[buf]
  if diffstat then return diffstat end
  vim.system({
    'git',
    '-C',
    dir,
