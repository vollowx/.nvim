...
local fterm_opts_default = {
  termopts = {
    ...
    event_callbacks = {
        ...
        auto_resize = {
          ...
          callback = function(term)
            vim.api.nvim_create_autocmd('VimResized', {
              ...
              callback = function() term:update_win_size() end,
            })
          end,
        },
      },
      on_open = {
        auto_insert = {
          priority = 100,
          callback = function(_) vim.cmd.startinsert() end,
        },
        prevent_shifting = {
          priority = 100,
          callback = function(term) term:right_shift() end,
        },
      },
    },
  },
  jobopts = {
    on_exit = function(jobid, code, event)
      ...
      if term and code == 0 and event == 'exit' then term:del() end
    end,
  },
  winopts = {
    relative = 'editor',
...
local function normalize_relative_geometry(number, base, rounding)
  ...
  if 0 < number and 1 > number then return math[rounding](number * base) end
...
local function normalize_fterm_winopts(winopts)
  ...
  -- stylua: ignore end
  if normalized_winopts.relative ~= 'win' then normalized_winopts.win = nil end
...
local function normalize_fterm_jobopts(jobopts)
  ...
  normalized_jobopts =
    vim.tbl_deep_extend('force', fterm_opts_default.jobopts, normalized_jobopts) --[[@as table]]
...
function fterm_t:del()
  ...
  if self.jobid then vim.fn.jobstop(self.jobid) end
  ...
  if event_callbacks then
    ...
    for _, callbacks in pairs(sorted_callbacks) do
      for _, callback in pairs(callbacks) do
        if vim.is_callable(callback) then callback(self) end
      end
    end
  end
end
...
---@param jobid integer
---@return fterm_t?
function fterm_t:get_by_job(jobid) return fterm_list_by_job[jobid] end

---Get the terminal instance by bufnr
---@param buf integer
---@return fterm_t?
function fterm_t:get_by_buf(buf) return fterm_list_by_buf[buf] end
...
function fterm_t:win_is_visible(tab)
  ...
  if not self.win or not self.win[tab] then return false end
...
function fterm_t:open()
  if self:win_is_visible() then return end
...
function fterm_t:close(tab)
  tab = tab or vim.api.nvim_get_current_tabpage()
  if not self:win_is_visible(tab) then return end
...
function fterm_t:update_win_size()
  if not self:win_is_visible() then return end
