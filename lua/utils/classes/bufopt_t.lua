...
function bufopt_t:new(name, default)
  if bufopts[name] then return bufopts[name] end
  vim.g[name] = default
  ...
  vim.api.nvim_create_autocmd({ 'BufDelete', 'BufWipeOut', 'BufUnload' }, {
    group = new_opt.augroup,
    callback = function(info) new_opt.initialized[info.buf] = nil end,
  })
...
---@return any
function bufopt_t:get(buf) return vim.b[buf or 0][self.name] end

---Get the buffer option, mimic vim :setlocal <option>?
---@param buf number? buffer number
---@return any
function bufopt_t:getlocal(buf) return vim.b[buf or 0][self.name] end

---Get the buffer option global value, mimic vim :setglobal <option>?
---@return any
function bufopt_t:getglobal() return vim.g[self.name] end

---Print the buffer option, mimic vim :set <option>?
---@param buf number? buffer number
function bufopt_t:print(buf) vim.notify(vim.inspect(self:get(buf))) end

---Print the buffer option, mimic vim :setlocal <option>?
---@param buf number? buffer number
function bufopt_t:printlocal(buf) vim.notify(vim.inspect(self:getlocal(buf))) end

---Print the buffer option global value, mimic vim :setglobal <option>?
---@return any
function bufopt_t:printglobal() vim.notify(vim.inspect(self:getglobal())) end
...
---@param value any value to set the buffer option to
---@param buf number? buffer number
function bufopt_t:setlocal(value, buf) vim.b[buf or 0][self.name] = value end

---Set buffer option global value, mimic vim :setglobal
---@param value any value to set the buffer option to
function bufopt_t:setglobal(value) vim.g[self.name] = value end

---Toggle buffer option, mimic vim :set <option>!
---@param buf number? buffer number
function bufopt_t:toggle(buf) self:set(not self:get(buf), buf) end

---Toggle buffer option, mimic vim :setlocal <option>!
---@param buf number? buffer number
function bufopt_t:togglelocal(buf) self:setlocal(not self:getlocal(buf), buf) end

---Toggle buffer option global value, mimic vim :setglobal <option>!
function bufopt_t:toggleglobal() self:setglobal(not self:getglobal()) end

---Reset buffer option to default value, mimic vim :set <option>&
---@param buf number? buffer number
function bufopt_t:reset(buf) self:set(self.default, buf) end

---Reset buffer option local value to default value,
---mimic vim :setlocal <option>&
---@param buf number? buffer number
function bufopt_t:resetlocal(buf) self:setlocal(self.default, buf) end

---Reset buffer option global value to default value,
---mimic vim :setglobal <option>&
function bufopt_t:resetglobal() self:setglobal(self.default) end
...
function bufopt_t:scope_action(opts, action, ...)
  if opts['global'] then return self[action .. 'global'](self, ...) end
  if opts['local'] then return self[action .. 'local'](self, ...) end
